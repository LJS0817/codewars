...
public class Kata 
{
  public static string GenerateBC(string url, string separator)
  {
    ...
    int len = str.Length;
    string[] ig = new string[] {"the","of","in","from","by","with","and", "or", "for", "to", "at", "a"};
    
    const string lastIndex = "index.";
    if((len > 1 && str[len - 1].IndexOf(lastIndex) > -1 && str[len - 1].Length > lastIndex.Length) || str[len - 1].Length == 0 || str[len - 1][0] == '?' || str[len - 1][0] == '#') {
      len--;
    }
    ...
    string href = "";
    for(int i = 0; i < len; i++) {
      tag = i == 0 ? "HOME" : (str[i].IndexOf('.') > -1 ? str[i].Substring(0, str[i].IndexOf('.')) : str[i]).Replace('-', ' ').ToUpper();
      href = i == 0 ? "/" : (i > 1 ? $"{href}{str[i]}/" : $"/{str[i]}/");
      if(tag.IndexOf('#') > -1) tag = tag.Substring(0, tag.IndexOf('#'));
      if(tag.IndexOf('?') > -1) tag = tag.Substring(0, tag.IndexOf('?'));
      if(tag.Length > 30) {
        string tmp = "";
        foreach(string s in tag.Split(' ')) {
          if(ig.Contains(s.ToLower())) continue;
          tmp += s[0] + "";
        }
        tag = tmp;
      }
      if(i < len - 1) {
        rst += $"<a href=\"{href}\">{tag}</a>{separator}";
      } else {
        rst += $"<span class=\"active\">{tag}</span>";
      }
    }
    // Do the things
    return rst;
