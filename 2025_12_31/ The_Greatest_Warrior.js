...
class Warrior {
  ...
  experience() { return this._exe; }
  rank() { return ranks[this._rank] }
  achievements() { return this._ac; }
  levelUp(exe) {
    this._exe += exe;
    if(this._exe > MAX_EXE) this._exe = MAX_EXE;
    
    this._level = parseInt(this._exe / 100);
    this._rank = parseInt(this._level / 10);
  }
  
  training (trainInfo) { 
    if(trainInfo[2] > this._level) return "Not strong enough";
    this._ac.push(trainInfo[0]);
    this.levelUp(trainInfo[1])
    return trainInfo[0];
  }
  
  battle(enemyLevel) {
    if(enemyLevel < 1 || enemyLevel > 100) return "Invalid level";
    let d = enemyLevel - this._level;
    if(parseInt(enemyLevel / 10) > this._rank && d >= 5) return "You've been defeated";
    ...
    if(d < -1) return "Easy fight";
    else if(d <= 0) return "A good fight";
